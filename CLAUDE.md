...
| `exercise` | Exercise library entries per day | `dayNumber`, `name`, `equipment`, `variations[]` (`{name, default, targetWeight/Reps/Sets}`) |
| `logged-workout` | A completed workout session | `userId`, `dayNumber`, `date`, `mode` (quick/detailed), `exercises[]` (`{name, variation, weight, reps, sets}`) |
| `cardio-session` | A completed cardio session | `userId`, `date`, `activity` (treadmill/bike), `durationMinutes`, `treadmill{}`, `bike{}` |
