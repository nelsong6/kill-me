...
resource "azurerm_dns_cname_record" "workout" {
  ...
  zone_name           = data.terraform_remote_state.infra.outputs.dns_zone_name
...
resource "azurerm_static_web_app_custom_domain" "workout" {
  ...
  domain_name       = "${local.front_app_dns_name}.${data.terraform_remote_state.infra.outputs.dns_zone_name}"
